...
vertex minheap_graph::getv(int i)
{
    return v[i];
}

//Υλοποιούμε τον default constructor της minheap_graph
minheap_graph::minheap_graph ()
{
    v=NULL;
    keys=NULL;
    position_in_vertices=NULL;
    size=0;
}

int minheap_graph::getsize() //Δημιουργούμε μία συνάρτηση getsize η οποία επιστρέφει το size γιατί το size είναι private μεταβλητή
{
    return size;
}

vertex minheap_graph::findmin() //Δημιουργούμε μία συνάρτηση findmin η οποία επιστρέφει το περιεχόμενο της θέσης 0 του πίνακα v γιατί αυτή είναι η τιμή του ελάχιστου στοιχείου του σωρού
{
    return v[0];
}

void minheap_graph::sort(int position) //Η συνάρτηση sort φροντίζει έτσι ώστε κάθε φορά ο γονέας του στοιχείου της θέσης position στον σωρό να έχει μικρότερη τιμή keys από την τιμή keys του παιδιού. Αν αυτό δεν ισχύει τότε τα αντιμεταθέτει και επαναλαμβάνει τον έλεγχο για το στοιχείο που είναι πλέον γονέας. Αλλιώς επιστρέφει.
{
    int temp;
    vertex temp2;
    ...
    else //Αλλιώς
    {
        ...
        temp=position_in_vertices[position];
        position_in_vertices[position]=position_in_vertices[(position-1)/2];
        position_in_vertices[(position-1)/2]=temp;
...
void minheap_graph::insert(vertex v1,int key,int position) //Δημιουργούμε μία συνάρτηση insert η οποία εισάγει ένα στοιχείο στην κατάλληλη θέση του στον σωρό ελαχίστων. Οι πληροφορίες του στοιχείου αυτού δίνονται ως παράμετροι.
{
    ...
    for(i=0;i<size;i++)
    {
        temp[i]=v[i];
        temp2[i]=keys[i];
        temp3[i]=position_in_vertices[i];
    }
    ...
    delete []v;
    delete []keys;
    delete []position_in_vertices;
    
    v=temp;
    keys=temp2;
    position_in_vertices=temp3;
    
    sort(i);
    size++;
}

vertex minheap_graph::deletemin() //Δημιουργούμε μία συνάρτηση deletemin η οποία διαγράφει το ελάχιστο στοιχείο από τον σωρό ελαχίστων. Αυτό το πραγματοποιεί με την βοήθεια της συνάρτησης sortdeleted
{
    vertex r;
    r=v[0];
    ...
    if(right_child_position<size)
    {
        if(keys[left_child_position]<=keys[right_child_position]) //Αν από τα δύο παιδιά αυτό με τη μικρότερη τιμή είναι το αριστερό τότε κάνε το αριστερό παιδί γονέα, κάνε τον γονέα αριστερό παιδί και πήγαινε να ελέγξεις αν ο γονέας που έγινε αριστερό παιδί έχει τιμή μικρότερη από τα νέα παιδιά του (αν έχει)
        {
            if(keys[left_child_position]<keys[parent_position]) //Η παραπάνω διαδικασία αν το αριστερό παιδί έχει μικρότερη τιμή ή τα δύο παιδιά έχουν την ίδια τιμή 
            {
                ...
                temp=position_in_vertices[left_child_position];
                position_in_vertices[left_child_position]=position_in_vertices[parent_position];
                position_in_vertices[parent_position]=temp;
        ...
        else
        {
            if(keys[right_child_position]<keys[parent_position]) //Η παραπάνω διαδικασία αν το δεξί παιδί έχει μικρότερη τιμή 
            {
                ...
                temp=position_in_vertices[right_child_position];
                position_in_vertices[right_child_position]=position_in_vertices[parent_position];
                position_in_vertices[parent_position]=temp;
...
void minheap_graph::decreasekey(vertex v1,int key) //Η συνάρτηση decreasekey εντοπίζει την κορυφή που δέχεται ως πρώτη παράμετρο στον σωρό, και σε εκείνο το στοιχείο του σωρού αποθηκεύει στο keys την 2η παράμετρο.
{
    int i;
