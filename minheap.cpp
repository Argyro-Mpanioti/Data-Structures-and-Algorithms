...
void minheap::build (const char *filename)
{
    ...
    if(g.is_open()) //Ελέγχουμε αν το αρχείο άνοιξε επιτυχώς
    {
        ...
        while(g>>timh) //Όσο το αρχείο έχει γραμμές που δεν έχουν διαβαστεί αποθήκευσε την επόμενη γραμμή του στον string timh
        {
...
void minheap::insert(int number) //Δημιουργούμε μία συνάρτηση insert η οποία εισάγει ένα στοιχείο στην κατάλληλη θέση του στον σωρό ελαχίστων. Το στοιχείο αυτό έχει τιμή που δίνεται ως παράμετρος στην συνάρτηση
{
    ...
    while(value[thesh_neou_stoixeiou]<value[thesh_gonea])//Όσο το νέο στοιχείο έχει μικρότερη τιμή από τον γονέα του
    {
...
void minheap::sortdeleted(int goneas) //Η sortdeleted δέχεται ως παράμετρο τη θέση ενός γονέα στον πίνακα value και ελέγχει αν το  παιδί με τη μικρότερη τιμή έχει τιμή μικρότερη από την τιμή του γονέα. Αν ναι, τότε τις αντιμεταθέτει και επαναλαμβάνει τον έλεγχο για τον νέο γονέα του και τα παιδιά του (αν έχει). Αν όχι τότε επιστρέφει.
{
    ...
    if(dpaidi<size)
    {
        if(value[apaidi]<=value[dpaidi]) //Αν από τα δύο παιδιά αυτό με τη μικρότερη τιμή είναι το αριστερό τότε κάνε το αριστερό παιδί γονέα, κάνε τον γονέα αριστερό παιδί και πήγαινε να ελέγξεις αν ο γονέας που έγινε αριστερό παιδί έχει τιμή μικρότερη από τα νέα παιδιά του (αν έχει)
        {
            if(value[apaidi]<value[goneas]) //Η παραπάνω διαδικασία αν το αριστερό παιδί έχει μικρότερη τιμή ή τα δύο παιδιά έχουν την ίδια τιμή 
            {
                temp=value[apaidi];
        ...
        {
            if(value[dpaidi]<value[goneas]) //Η παραπάνω διαδικασία αν το δεξί παιδί έχει μικρότερη τιμή 
            {
                temp=value[dpaidi];
